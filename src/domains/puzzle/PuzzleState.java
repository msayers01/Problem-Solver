...
public class PuzzleState extends State {
    ...
    public static PuzzleState Copy(PuzzleState state) {
        return new PuzzleState(state.getTiles());
    }
    /*
    ...
    */
    ...
    @Override
        public int hashCode() {
            int hash = 7;
            hash += 9 * Arrays.deepHashCode(this.getTiles());
            return hash;
        }
...
//        return tilesOutOfPlace(this, goal);
	...
	// compute the count
        ...
        PuzzleState end = (PuzzleState)goal;
        
        for(int i = 1; i <= 8; i++)
        {
            Location currLoc = curr.getLocation(i);
            Location goalLoc = end.getLocation(i);
	...
	// compute the sum
        ...
        for(int i = 1; i <= 8; i++)
        {
            Location currLocation = curr.getLocation(i);
            Location goalLocation = end.getLocation(i);
            
            if(!currLocation.equals(goalLocation))
            {
                sum += Math.abs(goalLocation.getRow() - currLocation.getRow() ) +
                       Math.abs(goalLocation.getColumn() - currLocation.getColumn() );
